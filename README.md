...
The Cockpit exposure tabs (asset class, positions, country, industry, company and region) include 1W, 1M, 3M and 1Y price-movement columns. Position changes come from verified exact-ISIN Yahoo chart closes. Exposure changes are an indicative weight-averaged movement of the linked quoted instruments, shown only with at least 50% quote coverage and compatible comparison dates; fund-level price moves are not country or sector benchmark returns. Missing or stale quotes show a dash. These figures exclude distributions/dividends, fees, cash flows and currency conversion, and never revalue imported positions or claim to be client investment returns.
