...
export function materialEvent(title: string): MaterialEvent | undefined {
  ...
  if (/\b(trading (?:is |was )?(?:halted|suspended)|dividend (?:cut|suspended)|cuts? (?:its )?(?:guidance|forecast)|fraud (?:charges|investigation)|recalls?)\b/i.test(title)) return { label: 'Material event — verify', severity: 'review' };
  return undefined;
}
