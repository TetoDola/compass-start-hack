...
export function advisorFacts(a: Analysis, context?: MarketContext, selection?: EventDiscussion): AnswerBlock[] {
  ...
  for (const range of ranges) {
    ...
    blocks.push({ id:`performance:${range}`, title:`${range} portfolio development`, text:p.available ? [`Portfolio value ${p.change! >= 0 ? 'rose' : 'fell'} ${percent(Math.abs(p.change!))} (${money(Math.abs(p.amount!),a.historyCurrency)}) from ${dateLabel(p.start!.date,true)} to ${dateLabel(p.end!.date,true)}.`, `${money(p.start!.value,a.historyCurrency)} → ${money(p.end!.value,a.historyCurrency)}. This is value movement, including possible cash flows, not investment return. No position-level attribution is supplied.`] : [`${range} change is unavailable: there are no matching start/end observations. The case history is monthly; no values are interpolated.`, ...(p.end ? [`Latest supplied observation: ${money(p.end.value,a.historyCurrency)} on ${dateLabel(p.end.date,true)}.`] : ['Select a portfolio with compatible value history.'])], evidence });
  }
  for (const kind of ['industry','country','company','region'] as const) {
    const rows = portfolioExposures(a,kind);
    blocks.push({ id:`exposure:${kind}`, title:`Exposure by ${kind}`, text:[a.weightsAvailable ? `${percent(rows.reduce((n,r)=>n+r.weight,0))} of the portfolio is attributed in this view. Weights use the whole portfolio, including the uncovered portion, as denominator.` : 'Select a non-overlapping portfolio to calculate exposure weights.', kind==='country' ? 'Direct reference countries and country-specific fund categories only. Broad regions and fund domicile are not substituted for underlying countries.' : kind==='company' ? 'Direct equity plus published top ten fund constituents. Exact ISINs merge; different share classes remain separate. Indirect weights are estimates from dated snapshots.' : kind==='industry' ? 'Direct security classifications plus supplied fund industry breakdowns.' : 'Supplied fund regions only; distinct from the individual country breakdown.'], rows: rows.slice(0,10).map(r=>({name:r.name,value:percent(r.weight,1),newsId:r.id})), evidence: rows.slice(0,10).flatMap(r=>r.evidence) });
  }
