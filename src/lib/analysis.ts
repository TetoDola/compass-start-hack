...
export function analyze(dataset: Dataset, customer: Row, scope = 'all'): Analysis {
  ...
  return { riskProfile: dataset.reference.RiskProfiles.find(r=>r.Id===customer.RiskProfileId), esgProfile: list(dataset.reference.EsgProfiles).find(e=>e.Id===customer.EsgProfileId), policyTargets: portfolios.map(p=>({portfolioId:p.PortfolioId,name:p.PortfolioNr,mappings:list(dataset.reference.StrategicAssetAllocations.find(s=>s.Id===p.StrategicAssetAllocationId)?.Mappings)})), customer, portfolios, scope, currency, aum, liquidity, scopeAmbiguous, weightsAvailable, holdings, findings, evidence, history, historyCurrency, allocations: [...allocations].map(([label, weight]) => ({ label, weight })).sort((a, b) => b.weight - a.weight), fundCoverage, fundSectors, notes, proposals, violations, unresolved, warnings, summary, strategy: strategyNames.length === 1 ? strategyNames[0] : strategyNames.length > 1 ? 'Multiple strategies' : 'Not recorded' };
}
