...
export function analyze(dataset: Dataset, customer: Row, scope = 'all'): Analysis {
  ...
  for (const p of portfolios) {
    ...
    for (const [i, s] of list(p.SecurityPositions).entries()) {
      ...
      const holding: Holding = { id, name: source.title, securityId: s.SecurityId, portfolioId: p.PortfolioId, portfolio: p.PortfolioNr || String(p.PortfolioId), currency: p.PortfolioCurrency || currency, value: number(s.TotalAmountInPortfolioCurrency), weight, asset: master?.SAA_AssetClassName || 'Not classified', sector: master?.IndustryName || 'Not classified', country: master?.CountryName, known: !!master, evidence: source, priceDate: master?.PriceDateUtc || s.PriceDateUtc, priceStale: !validDate(master?.PriceDateUtc || s.PriceDateUtc) || Date.now() - Date.parse(master?.PriceDateUtc || s.PriceDateUtc) > 30 * 86400000, instrumentType, displayName: fundHoldings.get(isin)?.name || displayInstrumentName(source.title, instrumentType), isin, fundBreakdown: funds.get(s.SecurityId), fundHoldings: instrumentType === 'Investment fund' ? fundHoldings.get(isin) : undefined, riskContribution: number(s.ContributionVolatility) ?? undefined };
