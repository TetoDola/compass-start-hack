...
export function EvidenceDrawer({ evidence, onClose }: { evidence: Evidence; onClose: () => void }) {
  ...
  return <div className="drawer-overlay" onClick={onClose}><aside ref={drawerRef} className="evidence-drawer" role="dialog" aria-modal="true" aria-labelledby="evidence-title" onClick={e => e.stopPropagation()}><div className="drawer-top"><span className="eyebrow"><ShieldCheck size={15} /> EVIDENCE</span><button ref={closeRef} className="icon-button" onClick={onClose} aria-label="Close evidence"><X size={20} /></button></div><div className="source-type">{evidence.type === 'calculation' ? 'Calculated from source data' : /^https?:\/\//.test(evidence.location) ? 'External source' : 'Supplied source record'}</div><h2 id="evidence-title">{evidence.title}</h2>{evidence.date && <p className="muted">Recorded {dateLabel(evidence.date)}</p>}<div className="evidence-fields">{evidence.fields.map((field, i) => <div key={i}><span>{field.label}</span><p>{field.value}</p></div>)}</div><div className="source-location"><FileText size={17} /><div><span className="eyebrow">SOURCE LOCATION</span><code>{/^https?:\/\//.test(evidence.location) ? <a href={evidence.location} target="_blank" rel="noreferrer">Open original source ↗</a> : evidence.location}</code></div></div>{evidence.note && <div className="source-note"><Fingerprint size={18} /><p>{evidence.note}</p></div>}<p className="drawer-footer">{/^https?:\/\//.test(evidence.location) ? 'External source with its own publication date. This is separate from the shifted case-data timeline.' : 'Source: UNRISKOMEGA case dataset. Most exported dates are shifted. Account identifiers are excluded.'}</p></aside></div>;
}
