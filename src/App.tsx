...
export default function App() {
  ...
  useEffect(()=>{
    ...
    fetch('/data/case-data.json').then(r=>{if(!r.ok)throw new Error('The case dataset could not be loaded. Run npm run prepare:data, then reload.');return r.json();}).then(async (original:Dataset)=>{
      let d=original;try{d=await loadImportedWorkspace()||original;}catch{if(alive){setImportMessage('Saved imports could not be loaded; showing the case dataset.');setImportError(true);}}
  ...
  async function importFile(file?:File){
    if(!file || !dataset)return;setImporting(true);setImportMessage('');setPdfError('');setImportProgress('Reading file…');
    try{
      if(file.size>25*1024*1024)throw new Error('Choose a PDF or JSON file smaller than 25 MB.');
      if(/\.pdf$/i.test(file.name)||file.type==='application/pdf'){
        ...
        const next={...dataset,clients:imported.clients,reference:imported.reference,version:`Imported · ${file.name}`};
        await saveImportedWorkspace(next);setDataset(next);selectCustomer(imported.clients[0].ClientId);setImportError(false);setImportMessage(`Imported ${imported.clients.length} clients from ${file.name}. ${imported.suppliedReference?'Uploaded reference data is active.':'Existing reference data is reused.'}`);
      }
  ...
  return <div className="workspace-app" data-theme={theme}>
    <header className="ws-topbar"><a className="ws-brand" href="#" onClick={e=>{e.preventDefault();window.scrollTo({top:0});}}><Compass size={22}/><span>compass<span>.</span></span></a><span className="ws-topbar-title">WEALTH INTELLIGENCE</span><div className="ws-topbar-actions"><button className="ws-button ws-button--primary ws-world-launcher" aria-label="Open World view" onClick={()=>setPanel('world')}><Globe2 size={15}/>World</button><span className="ws-data-connected"><i/>{imported?'Imported dataset':'Case workspace'}</span>{imported && <button className="ws-link" onClick={()=>void restoreCases()}>Restore cases</button>}<button className="ws-icon" aria-label={`Switch to ${theme==='dark'?'light':'dark'} theme`} onClick={()=>setTheme(theme==='dark'?'light':'dark')}>{theme==='dark'?<Sun size={16}/>:<Moon size={16}/>}</button><input ref={uploadRef} type="file" accept="application/json,.json,application/pdf,.pdf" className="visually-hidden" aria-label="Import client data (JSON or PDF)" onChange={e=>void importFile(e.target.files?.[0])}/><button className="ws-button" disabled={importing} onClick={()=>uploadRef.current?.click()}><Upload size={14}/>{importing?(importProgress||'Saving…'):'Import client data'}</button></div></header>
    ...
    <main className="ws-page">{importMessage && <div className={`ws-notice ${importError?'error':'success'}`} role={importError?'alert':'status'}>{importError?<Info size={16}/>:<Check size={16}/>}<p>{importMessage}</p><button className="ws-icon" aria-label="Dismiss import message" onClick={()=>setImportMessage('')}><X size={16}/></button></div>}
      ...
      <div className="ws-context-strip"><span>{analysis.strategy}<span className="ws-separator">/</span>Profile {dateLabel(customer.ProfilingDateUtc,true)}</span><span>{latestFactory?`Record snapshot ${dateLabel(latestFactory,true)}`:'Snapshot date unavailable'} · {analysis.portfolios.some(p=>p.ExternalSource)?'External statement included':'Case dates shifted'}</span></div>
