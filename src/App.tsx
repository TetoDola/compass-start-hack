...
export default function App() {
  ...
  if(loadError)return <div className="boot-screen"><img className="boot-brand" src="/assets/logos/uro-tldr-light.svg" alt="URO TL;DR"/><h1>Let’s reconnect the data.</h1><p>{loadError}</p><button className="button primary" onClick={()=>location.reload()}>Retry loading</button></div>;
  if(!dataset || !analysis || !customer || !selected)return <div className="boot-screen"><img className="boot-brand loading-brand" src="/assets/logos/uro-tldr-light.svg" alt="URO TL;DR"/><h1>Preparing your workspace</h1><p>Connecting customer records and portfolio context…</p></div>;
