...
import { buildNetwork, neighborhood, type NetworkNode, type NetworkEdge } from './lib/network';
import { instrumentId, type MarketContext } from './lib/briefing';
...
export function CustomerGraph({ analysis, selected, overview, onOverview, onSelect, onEvidence, onBrief, fundStatus, onRefreshFund, context, contextFocus }: {
  ...
  fundStatus: Record<string, FundLookupState>; onRefreshFund: (isin: string) => void; context?: MarketContext; contextFocus?: string;
}) {
  ...
  const visible = useMemo(() => {
    ...
    return local && nodeId ? neighborhood(filtered, new Set([nodeId]), 1) : filtered;
  ...
  return <section className="network-workspace">
    ...
    <aside className="network-inspector">{node ? <><div className="inspector-label"><span className="eyebrow">{node.materialEvent ? 'MATERIAL HEADLINE — VERIFY' : node.type === 'company' ? 'COMPANY SECURITY' : node.type}</span><button className="icon-button" aria-label="Clear selection" onClick={() => select(null)}><X size={15}/></button></div><h3>{node.name}</h3><p>{node.description}</p>{node.isin && <code>{node.isin}</code>}<button className="button secondary inspector-focus" onClick={() => { setLocal(!local); onOverview(); }}>{local ? 'Show whole network' : 'Focus on connections'}</button>
      {node.type === 'fund' && <div className="fund-expansion"><h4>Inside this fund</h4>{node.snapshot ? <><p>{node.snapshot.holdings.length} published holdings · {dateLabel(node.snapshot.asOf, true)}</p><div className="coverage-track"><span style={{ width: `${node.snapshot.holdings.reduce((s, h) => s+h.weight, 0)*100}%` }}/></div><small>{percent(node.snapshot.holdings.reduce((s, h) => s+h.weight, 0))} covered · {percent(Math.max(0, 1-node.snapshot.holdings.reduce((s, h) => s+h.weight, 0)))} not shown</small><button className="button primary" onClick={() => setExpanded(old => { const next = new Set(old); if (next.has(node.id)) next.delete(node.id); else next.add(node.id); return next; })}>{expanded.has(node.id) ? 'Collapse companies' : 'Expand top 10 companies'}</button><a className="text-button" href={node.snapshot.sourceUrl} target="_blank" rel="noreferrer">{node.snapshot.sourceName} ↗</a></> : <p>{lookup?.status === 'loading' ? 'Looking up a dated holdings list…' : 'Constituents unavailable. Category exposures remain available where supplied.'}</p>}{node.isin && <button className="text-button" disabled={lookup?.status === 'loading'} onClick={() => onRefreshFund(node.isin!)}><RefreshCw size={13}/>{lookup?.status === 'loading' ? 'Fetching…' : 'Refresh holdings'}</button>}{lookup?.status === 'unavailable' && <p className="microcopy">{lookup.message} {node.snapshot ? 'The saved snapshot is retained.' : ''}</p>}</div>}
