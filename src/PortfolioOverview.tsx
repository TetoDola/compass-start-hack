...
export function PerformancePanel({ analysis, range, onRange }: { analysis: Analysis; range: TimeRange; onRange: (r: TimeRange) => void }) {
  ...
  return <section className="panel performance-panel"><div className="panel-heading"><div><span className="eyebrow">HOW THE PORTFOLIO HAS CHANGED</span><h3>Portfolio value</h3></div><RangeControl value={range} onChange={onRange} label="Portfolio history period"/></div>
...
export function AttentionPanel({ analysis, context, onEvidence, onGraph, onContextGraph }: { analysis: Analysis; context?: MarketContext; onEvidence: (e: Evidence) => void; onGraph: (id: string) => void; onContextGraph: (id: string) => void }) {
  ...
  const items = [...events, ...portfolioAttention(analysis)].sort((a,b) => (a.id === 'scope' ? -1 : rank[a.level])-(b.id === 'scope' ? -1 : rank[b.level]));
...
export function ExposurePanel({ analysis, context, onEvidence, onNews }: { analysis: Analysis; context?: MarketContext; onEvidence: (e: Evidence) => void; onNews: (id: string, name: string) => void }) {
  ...
  return <section className="panel exposure-panel"><div className="panel-heading"><div><span className="eyebrow">WHERE THE MONEY IS EXPOSED</span><h3>Investments & connected news</h3></div></div><div className="exposure-tabs" role="group" aria-label="Exposure dimension">{(['industry','country','company','region'] as const).map(k => <button key={k} aria-pressed={k===kind} onClick={() => {setKind(k);setAll(false);}} style={{ '--dimension': dimensionColor(k) } as React.CSSProperties}><i/>{k === 'region' ? 'Fund regions' : k}</button>)}</div>
    {!analysis.weightsAvailable ? <p className="muted">Choose a non-overlapping portfolio to calculate weights.</p> : <><p className="exposure-coverage"><strong>{percent(total)}</strong> of portfolio attributed · {kind==='company' ? 'Direct equity + published top 10 fund holdings, joined by ISIN.' : kind==='country' ? 'Reference country for direct securities + country-specific fund categories. Broad regions are not guessed.' : kind==='industry' ? 'Direct classifications + supplied fund industry breakdowns.' : 'Supplied fund regions only; separate from the country table.'}</p>
