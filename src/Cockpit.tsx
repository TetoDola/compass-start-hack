...
import { ArrowUpRight, FileText, Minus, MessageSquare } from 'lucide-react';
...
export function Cockpit({ analysis, dataset, context, phase, range, onRange, onEvidence, onNews, onWorkspace, onRefresh }: {
  analysis: Analysis; dataset: Dataset; context?: MarketContext; phase: string; range: TimeRange;
  onRange: (r: TimeRange) => void; onEvidence: (e: Evidence) => void; onNews: (id: string, name: string) => void;
  onWorkspace: () => void; onRefresh: () => void;
}) {
  ...
  return <div className="cockpit">
    ...
    <div className={`cockpit-chat ${chatOpen ? 'open' : ''}`}>
      {chatOpen
        ? <><div className="cockpit-chat-bar"><MessageSquare size={15} /><strong>Adviser chat</strong><span>{analysis.customer.ClientRef}{focused ? ` · ${focused.name} in scope` : ''}</span><button className="icon-button" aria-label="Minimise the chat window" onClick={() => setChatOpen(false)}><Minus size={16} /></button></div>
          <AdvisorChat compact key={`${analysis.customer.ClientId}:${analysis.scope}`} analysis={analysis} context={context} phase={phase} onWorkspace={onWorkspace} onEvidence={onEvidence} onNews={onNews} onRefresh={onRefresh} /></>
        : <button className="cockpit-chat-launcher" onClick={() => setChatOpen(true)}><MessageSquare size={16} /><span>Ask about this customer<small>{context ? `${context.checked}/${context.requested} news searches complete` : 'Preparing the brief…'}</small></span></button>}
    </div>
  </div>;
}
