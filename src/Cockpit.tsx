...
import { linkedHoldingMove, marketPeriods, type MarketMove, type MarketPeriod } from './lib/marketPerformance';
...
export function Cockpit({ analysis, dataset, context, range, onRange, ...navigation }: {
  ...
  const moves = useMemo(() => new Map(market.quotes.filter(quote => quote.state === 'available' && quote.movements?.[marketPeriod]).map(quote => [quote.isin, quote.movements![marketPeriod]!])), [market.quotes, marketPeriod]);
    ...
    <div className="cockpit-top">
      ...
      <div className="cockpit-centre">
        ...
        <section className="panel cockpit-exposures" aria-label="Portfolio exposure">
          ...
          <div className="cockpit-price-period"><span>Linked price movement</span><div role="group" aria-label="Price movement period">{marketPeriods.map(value => <button key={value} aria-pressed={marketPeriod === value} onClick={() => setMarketPeriod(value)}>{value}</button>)}</div></div>
          {tab === 'asset' ? <AssetTable analysis={analysis} moves={moves} period={marketPeriod} onEvidence={navigation.onEvidence} /> : tab === 'positions' ? <PositionsTable analysis={analysis} moves={moves} period={marketPeriod} onEvidence={navigation.onEvidence} onGraph={navigation.onGraph} /> : <BreakdownSection key={tab} dimension={dimensions.find(d => d.kind === tab)!} rows={rows[tab]} analysis={analysis} moves={moves} period={marketPeriod} focus={focus?.id} onFocus={id => setFocus(f => f?.id === id ? null : { kind: tab, id })} />}
          <p className="cockpit-price-note">{market.loading ? 'Loading dated price histories… ' : ''}Position moves use the verified instrument’s published closes. Exposure moves are a weighted average of linked holdings with at least 50% quote coverage; they are not country/industry index or client investment returns. Missing, stale or mismatched dates show —.</p>
...
function MoveCell({ move, period, coverage }: { move?: MarketMove; period: MarketPeriod; coverage?: number }) {
  return <span className={`numeric cockpit-move ${move ? move.change < 0 ? 'negative' : 'positive' : 'unavailable'}`} title={move ? `${period} published price move from verified Yahoo Finance closes${coverage == null ? '' : ` · ${(coverage * 100).toFixed(0)}% of this exposure covered`}${move.start && move.end ? ` · ${move.start} to ${move.end}` : ''}. Excludes dividends, fees and FX conversion.` : 'No sufficiently covered, dated price comparison available.'}>{move ? `${move.change >= 0 ? '+' : '−'}${Math.abs(move.change).toFixed(1)}%` : '—'}</span>;
}

function exposureMove(parts: { id: string; weight: number }[], total: number, analysis: Analysis, moves: Map<string, MarketMove>): { change: number; coverage: number } | null {
  const holdings = new Map(analysis.holdings.map(holding => [holding.id, holding]));
  return linkedHoldingMove(parts.map(part => ({ weight: part.weight, move: moves.get(holdings.get(part.id)?.isin || '') })), total);
}

function AssetTable({ analysis, moves, period, onEvidence }: { analysis: Analysis; moves: Map<string, MarketMove>; period: MarketPeriod; onEvidence: Navigation['onEvidence'] }) {
  ...
  return <div className="cockpit-table-body"><div className="cockpit-asset-table" aria-label="Asset class allocation"><div className="cockpit-table-head"><span>Asset class</span><span>Allocation</span><span className="numeric">Weight</span><span className="numeric">{period} move</span></div>{analysis.allocations.map(row => { const linked = exposureMove(analysis.holdings.filter(holding => holding.asset === row.label), row.weight, analysis, moves); return <button className="cockpit-table-row" key={row.label} onClick={() => onEvidence(source)}><span>{row.label}</span><span className="cockpit-cell-bar"><i><b style={{width:`${Math.min(100, row.weight * 100)}%`}}/></i></span><strong className="numeric">{percent(row.weight, 1)}</strong><MoveCell move={linked ? { change: linked.change, start: '', end: '' } : undefined} period={period} coverage={linked?.coverage}/></button>; })}</div>{!analysis.allocations.length && <p className="muted">No asset-class allocation supplied.</p>}<p className="microcopy">Security and account weights from the selected snapshot. Select a row to inspect the calculation.</p></div>;
}

function PositionsTable({ analysis, moves, period, onEvidence, onGraph }: { analysis: Analysis; moves: Map<string, MarketMove>; period: MarketPeriod; onEvidence: Navigation['onEvidence']; onGraph: Navigation['onGraph'] }) {
  const [all, setAll] = useState(false), products = aggregateProducts(analysis);
  return <div className="cockpit-table-body"><div className="cockpit-position-table" aria-label="Positions by portfolio weight"><div className="cockpit-table-head"><span>Investment</span><span className="numeric">Value</span><span className="numeric">Weight</span><span className="numeric">{period} move</span><span className="visually-hidden">Connections</span></div>{(all ? products : products.slice(0, 6)).map(product => <div className="cockpit-table-row" key={product.id}><button className="cockpit-position-name" onClick={() => onEvidence(productEvidence(analysis, product))}><strong>{product.name}</strong><small>{lookThrough(product.positions[0]).label}{product.positions.length > 1 ? ` · ${product.positions.length} positions` : ''}</small></button><span className="numeric">{analysis.scopeAmbiguous ? '—' : money(product.value, product.currency)}</span><strong className="numeric">{analysis.weightsAvailable ? percent(product.weight, 1) : '—'}</strong><MoveCell move={moves.get(product.positions[0].isin || '')} period={period}/><span><button className="ws-icon" aria-label={`Explore ${product.name} in graph`} onClick={() => onGraph(product.id)}><Network size={14}/></button></span></div>)}</div>{!products.length && <p className="muted">No security positions supplied.</p>}{products.length > 6 && <button className="text-button cockpit-show-more" onClick={() => setAll(value => !value)}>{all ? 'Show top six' : `View all ${products.length} investments`} <ChevronDown size={12}/></button>}<p className="microcopy">Same-ISIN positions are combined. Cash is shown under Asset class.</p></div>;
}

function BreakdownSection({ dimension, rows, analysis, moves, period, focus, onFocus }: { dimension: typeof dimensions[number]; rows: BreakdownRow[]; analysis: Analysis; moves: Map<string, MarketMove>; period: MarketPeriod; focus?: string; onFocus: (id: string) => void }) {
  const [all, setAll] = useState(false);
  ...
  return <div className="cockpit-table-body"><div className="cockpit-breakdown-table" aria-label={`Exposure by ${dimension.title.toLowerCase()}`}><div className="cockpit-table-head"><span>{dimension.title}</span><span className="numeric">Weight</span><span className="numeric">{period} move</span><span className="numeric">Vs target</span><span className="numeric">News</span></div>{(all ? rows : rows.slice(0, 6)).map(row => { const linked = exposureMove(row.contributions, row.weight, analysis, moves); return <button aria-expanded={focus === row.id} className={`cockpit-table-row ${focus === row.id ? 'focused' : ''}`} key={row.id} onClick={() => onFocus(row.id)}><span className="cockpit-cell-name">{row.name}{row.violation && <small className={row.violation.Severity === 'Error' ? 'negative' : ''}>Recorded {row.violation.Severity === 'Error' ? 'breach' : 'warning'}</small>}{row.isin && dimension.kind === 'company' && !row.via.includes(row.name) && <small>via {row.via[0]}</small>}</span><strong className="numeric">{percent(row.weight, 1)}</strong><MoveCell move={linked ? { change: linked.change, start: '', end: '' } : undefined} period={period} coverage={linked?.coverage}/><span className={`numeric ${row.deviation == null ? 'unavailable' : row.deviation < 0 ? 'negative' : 'positive'}`}>{row.deviation == null ? '—' : `${row.deviation >= 0 ? '+' : '−'}${(Math.abs(row.deviation) * 100).toFixed(1)} pp`}</span><span className="numeric">{row.newsCount || '—'}</span></button>; })}</div>{!rows.length && <p className="muted">No classified exposure available for this dimension.</p>}{rows.length > 6 && <button className="text-button cockpit-show-more" onClick={() => setAll(value => !value)}>{all ? 'Show top six' : `View all ${rows.length}`} <ChevronDown size={12}/></button>}<p className="cockpit-coverage-line"><strong>{percent(attributed, 1)} attributed</strong><span>{percent(Math.max(0, securityWeight - attributed), 1)} of portfolio in unattributed securities</span></p><details className="cockpit-method"><summary>Coverage and target comparison</summary><p>{dimension.note} Weights use the whole selected portfolio. A dash means no comparable policy target was supplied. Targets are compared only when classifications cover the same positions and weights.</p></details></div>;
}
