...
import { createOutreachClassifier, validateOutreachRequest } from './outreach';
import { createCiiResolver } from './world-risk';
...
export async function selectBrief(candidates: BriefCandidate[], config: ProviderConfig): Promise<BriefResult> {
  ...
  try {
    const schema = { type: 'object', properties: Object.fromEntries(sections.map(({ id }) => [id, { type: 'array', items: { type: 'string', enum: candidates.filter(c => c.section === id && (id!=='health'||!candidates.some(c=>c.id==='scope')||c.id==='scope') && (id!=='actions'||!candidates.some(c=>c.id==='scope')||c.id==='agenda:actions') && (id!=='development' || !candidates.some(c=>c.id==='fact:value-development') || c.id==='fact:value-development')).map(c => c.id) }, minItems: 1, maxItems: id==='outlook' && candidates.some(c=>c.contextKind==='house-view')?2:1 }])), required: sections.map(s => s.id), additionalProperties: false };
...
export function intelligenceMiddleware(config: ProviderConfig) {
  const classifyOutreach = createOutreachClassifier(config);
  const context = createContextResolver(config), world = createWorldResolver(config), cii = createCiiResolver(config);
  return async (req: IncomingMessage, res: ServerResponse, next: () => void) => {
    const pathname = new URL(req.url || '/', 'http://localhost').pathname;
    if (!['/api/world-context', '/api/world-risk', '/api/market-context', '/api/briefing', '/api/advisor', '/api/outreach-classify'].includes(pathname)) return next();
    ...
    try {
      ...
      if (pathname === '/api/outreach-classify') {
        res.end(JSON.stringify({ results: await classifyOutreach(validateOutreachRequest(data.articles)) }));
      } else if (pathname === '/api/world-context') {
        res.end(JSON.stringify(await world(data.refresh === true)));
      } else if (pathname === '/api/world-risk') {
        res.end(JSON.stringify(await cii(data.refresh === true)));
        ...
        if (!Array.isArray(data.targets) || data.targets.length > 500 || data.targets.some((t: NewsTarget) => !t || typeof t.name !== 'string' || !t.name.trim() || t.name.length > 250 || typeof t.id !== 'string' || typeof t.via !== 'string' || (t.aliases != null && (!Array.isArray(t.aliases) || t.aliases.length > 30 || t.aliases.some(name => typeof name !== 'string' || !name.trim() || name.length > 250))) || (t.kind && !['company','industry','country','region'].includes(t.kind)) || (t.symbol && !/^[A-Za-z0-9.^=-]{1,32}$/.test(t.symbol)))) throw new Error('Invalid public instrument targets.');
