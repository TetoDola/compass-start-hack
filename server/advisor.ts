...
export async function selectAdvisorAnswer(input: AdvisorRequest, config: ProviderConfig) {
  ...
  try {
    ...
    return { ids: selected?['selected-event',...result.ids.filter((id:string)=>id!=='selected-event')].slice(0,5):result.ids as string[], mode:`${config.AI_PROVIDER === 'codex' ? 'Codex' : 'AI'} · verified record blocks` };
