...
export async function loadWorldLayers(config:ProviderConfig,request:typeof fetch=fetch):Promise<Pick<WorldDigest,'signals'|'quotes'|'chokepoints'|'layers'>> {
  ...
  const outcomes=await Promise.allSettled([
    ...
    (async()=>{const symbols=[['^GSPC','S&P 500'],['^VIX','VIX'],['GC=F','Gold'],['CL=F','WTI oil'],['NG=F','Natural gas']];const results=await Promise.allSettled(symbols.map(async([symbol,name],i)=>{await new Promise(r=>setTimeout(r,i*160));return parseQuote(await get(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1d&range=1mo`),name,now);}));quotes=results.flatMap(r=>r.status==='fulfilled'&&r.value?[r.value]:[]);layers.push({id:'markets',label:'Market observations',state:quotes.length===symbols.length?'available':quotes.length?'partial':'unavailable',count:quotes.length,message:'Latest provider observation; may be delayed. Own timestamps and session changes, independent of the case history.',sourceUrl:'https://finance.yahoo.com/markets/'});})(),
  ]);
